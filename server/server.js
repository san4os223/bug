...
var DEFAULT_NAME = 'Бій жуків';
var DEFAULT_MAP = 'random';
var LOBBY_SERVER = '8095';//localhost:8095
...
app.get('/add', (req, res) => {
	//if(req.params.name && req.params.kill && req.params.bug && req.params.item && req.params.email && req.params.password){
		user.addUser(req.params.name, req.params.kill, req.params.bug, req.params.item, req.params.email, req.params.password, res);
	//} else {
	//	res.send("USER NOT ADD");
	//}
});
...
app.get('/users', (req, res) => {
	if(req.param('id')){
		user.searchUser(req.param('id'), res);
	}else{
		user.sendUser(res);
	}

});//користувачі :localhost:3000/users
